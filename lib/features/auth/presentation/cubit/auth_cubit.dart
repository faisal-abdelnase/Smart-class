...
class AuthCubit extends Cubit<AuthState> {
  ...
  result.fold(
    ...
    ifRight: (user) {
      HiveService.box.put('role', user.role);
      emit(AuthSuccess(AuthStatus.login));
    },
  );
}
  ...
  result.fold(
    ...
    ifRight: (user) {
      HiveService.box.put('role', user.role);
      log("User role saved: ${user.role}"); // Log the saved role for debugging

      emit(AuthSuccess(AuthStatus.signup));
